...
for i in range(PROJECT_QUANTITY):
  project = {
    ...
    'start_date': fake.date_between(datetime.datetime.strptime('11-06-2024', '%d-%m-%Y'), datetime.datetime.strptime('31-12-2026', '%d-%m-%Y')),
    'end_date': fake.date_between(datetime.datetime.strptime('11-06-2024', '%d-%m-%Y'), datetime.datetime.strptime('31-12-2026', '%d-%m-%Y')),
    ...
    'team': random.choices(user_ids),
  }

  projects.append(project)


databaseConnection.executeQuery('DELET FROM project_user_rel')
databaseConnection.executeQuery('DELET FROM project_rate')
databaseConnection.executeQuery('DELET FROM comment')
databaseConnection.executeQuery('DELET FROM project_project_relation')
databaseConnection.executeQuery('DELET FROM project_ressource_rel')
databaseConnection.executeQuery('DELET FROM project')
